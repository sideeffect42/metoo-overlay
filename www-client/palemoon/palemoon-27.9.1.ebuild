...
src_install() {
	...
	cd "${extracted_dir}"
	einfo "Finding the package..."
	package_paths="$(find "${S}/${obj_dir}/dist" -iname "${P}.linux-*.tar.bz2" -print)"
	package_path=''
	for p in "${package_paths[@]}"; do
		if tar tjpf "${p}" | grep -q 'palemoon-bin$'; then
			package_path="${p}"
			break
		fi
	done
	[ -f "${package_path}" ] || die "Could not find package"
	einfo "Extracting the package..."
	tar xjpf "${package_path}"
