...
IUSE="data doc fltk gmp threads tls tune X"
...
PATCHES=( "${FILESDIR}"/${PN}-strip.patch
		"${FILESDIR}"/${PN}-ppc.patch
		"${FILESDIR}"/${P}-no-automagic.patch
		"${FILESDIR}"/${PN}-fltk-detection.patch
		)
...
src_configure() {
	...
	# sysdatadir installs a pari.cfg which is needed by sys-mathematics/gp2c
	./Configure \
		--prefix="${EPREFIX}"/usr \
		...
		$(use_with gmp) \
		$(usex tune '--tune' '') \
		$(usex X "$(use_with fltk)" '') \
		--graphic=$(usex X $(usex fltk fltk X11) none) \
		--mt=$(usex threads pthread single) \
